...
import os
os.environ["HF_TOKEN"] = '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'\

def gemma_2b(model ,tokenizer, device, Context_Prompt,Task_Prompt, question,question_id):

    str_first_two_caption = ','.join(Context_Prompt)
    input_text = "This is a question with its associated picture description and a set of similar question answers:"+"\nQuestion:"+question+'   '+"\nText description of the picture:"+str_first_two_caption+"Based on the given questions and the picture description, please summarize a picture description text that is useful to answer the question,The summary should be at least 50 words"    #33.42

    input_ids = tokenizer(input_text, return_tensors="pt").to(device)
    #tensor = tensor.to(device)
    outputs = model.generate(**input_ids,max_new_tokens=350)  #350
    #print(tokenizer.decode(outputs[0]))
    text=tokenizer.decode(outputs[0])
    generated_text_parts = text.split(input_text)

    generated_text_str = ' '.join(generated_text_parts)
...
def Qusetion_rank_2(model,tokenizer,question,caption):
    text=question
    pairs = [(text, a) for a in caption]
    with torch.no_grad():
        inputs = tokenizer(pairs, padding=True, truncation=True, return_tensors='pt', max_length=512).to("cuda:0")   #cuda
        scores = model(**inputs, return_dict=True).logits.view(-1, ).float()
        scores, indices = torch.topk(scores, k=30)  #
        #print(scores)
        top_pairs = []
        for i in range(30):
            idx = indices[i].item()
...
def load_model(model_selection,dist_selection):
    if (model_selection[:3] == 'opt' and dist_selection == 'hugging'):
        ...
        if model_selection == 'opt-66b':
            ...
            num_gpu = 4
            num_layer_first_gpu = 48
        elif model_selection == 'opt-13b':
            num_gpu = 2
        ...
        if model_selection not in ['opt-125m','opt-2.7b','opt-6.7b']:   #'opt-13b'
            ...
            num_layer_per_gpu = num_decoder_layer / num_gpu
            for i, layer in enumerate(device_map):
                device_map[layer] = int(i / num_layer_per_gpu)
            print(device_map)
...
def evaluation(model, test_data,caption_dict,syn_question_dict,syn_answer_dict,ans_to_cap_dicts,tokenizer, device, config, logger, writer, split='test',resume=False,result_dir = 'output/VQA_caption/result', result_filename = 'result'):
    ...
    # print(result)
    #lod_gemma
    device_gemma = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    #model = AutoModel.from_pretrained(model_name, revision="main", auth_token=token)
    tokenizer_2b = AutoTokenizer.from_pretrained("google/gemma-1.1-2b-it" )
    lod_model = AutoModelForCausalLM.from_pretrained("google/gemma-1.1-2b-it",torch_dtype=torch.bfloat16).to(device_gemma)
    #lod_rank
    device_rank=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    tokenizer_rank = AutoTokenizer.from_pretrained('BAAI/bge-reranker-large')
    model_rank = AutoModelForSequenceClassification.from_pretrained('BAAI/bge-reranker-large').to(device_gemma)
    model_rank.eval()
    ...
    for n, per_test_data in enumerate(metric_logger.log_every(test_data, print_freq, header)):
        ...
        print(type(syn_ans_queid))
        #for item in syn_ans_queid:
           # print(item)
        inputs = {"keys": {"question": question}}

        Prompt = "You are asked to select an appropriate answer from the potential answers in the context\n"
        ...
        #print(Task_Prompt)
        #formatted_data = [f"Question: {q}\nAnswer: {a}" for q, a in zip(syn_question_queid, syn_ans_queid)]
        #text=mini_model(caption,question)
        keyword=gemma_2b(lod_model, tokenizer_2b, device_gemma, caption,Task_Prompt ,question,question_id)
        result_dict = {
        ...
        """
        with open('ok_val_100.json', 'r') as f:
                    data = json.load(f)
        for item in data:
            if item['question_id'] == question_id:
                ...
                break
        """
        Task_Prompt_2=Qusetion_rank_2(model_rank,tokenizer_rank,question,caption)
        print(question)
        SCRA_VQA = "Contexts:" +Task_Prompt_2+"\n"+keyword +"\n"+Task_Prompt +"Question:" + question +  "\nAnswer:"
        ...
        SCRA_VQA_input = tokenizer(SCRA_VQA, padding='longest', truncation=True, return_tensors="pt").to(
            device)
        opt_input.input_ids = torch.cat((opt_input.input_ids, SCRA_VQA_input.input_ids), dim=-1)
        opt_input.attention_mask = torch.cat((opt_input.attention_mask, SCRA_VQA_input.attention_mask),
                                                  dim=-1)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            output_scores = output_scores.view(dim_0, -1).float()  #int(dim_1/config['max_answer_length'])201088
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size_test', default=64, type=int)   #64


    parser.add_argument('--num_caps_per_img', default=30, type=int)
    parser.add_argument('--num_question_per_img', default=60, type=int)
